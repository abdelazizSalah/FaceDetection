...
# I don't know why he has implemented it once again, it is already pre defined in utils file.
    ...
    for x in range(row):
        for y in range(column):
            ...
            # previous row + previous column - (previous column and row) + the current point
                ...
                integeral_img[x][y] = (
                    integeral_img[x-1][y] + integeral_img[x][y-1] - integeral_img[x-1][y-1]) + img[x][y]
    return integeral_img


# In[2]:

# These are the 5 types of the haar features so he defined them as an enum.
...
class HaarlikeFeatureDetector:
        ...
        for haartype in range(HaarlikeType.TYPES_COUNT.value):
            ...
            for x in range(0, width-hfw_sizeX+1):
                for y in range(0, height-hfw_sizeY+1):
                    ...
                    features_count += int((width-x)/hfw_sizeX) * \
                        int((height-y)/hfw_sizeY)
    ...
    def getSingleFeature(integralImg, haarType, x, y, w, h):
        ...
        if (haarType == HaarlikeType.TWO_HORIZONTAL):
            # negative
            white_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x, y, w/2, h)
            # positive regions
            black_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x + w/2, y, w/2, h)
        elif (haarType == HaarlikeType.TWO_VERTICAL):
            # negative
            white_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x, y, w, h/2)
            # positive regions
            black_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x, y+h/2, w, h/2)
        elif (haarType == HaarlikeType.THREE_HORIZONTAL):
            # negative
            white_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x, y, w/3, h) + HaarlikeFeatureDetector.getSumWindow(integralImg, x + 2*w/3, y, w/3, h)
            # positive regions
            black_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x + w/3, y, w/3, h)
        elif (haarType == HaarlikeType.THREE_VERTICAL):
            # negative
            white_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x, y, w, h/3) + HaarlikeFeatureDetector.getSumWindow(integralImg, x, y+2*h/3, w, h/3)
            # positive regions
            black_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x, y + h/3, w, h/3)
        elif (haarType == HaarlikeType.FOUR_DIAGONAL):
            white_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x, y, w/2, h/2) + HaarlikeFeatureDetector.getSumWindow(integralImg, x+w/2, y+h/2, w/2, h/2)
            black_region_sum = HaarlikeFeatureDetector.getSumWindow(
                integralImg, x+w/2, y, w/2, h/2) + HaarlikeFeatureDetector.getSumWindow(integralImg, x, y+h/2, w/2, h/2)
    ...
    def apply_features(features, training_data):
        ...
        for feature_description in features:
            def feature_extractor(intImg): return HaarlikeFeatureDetector.getSingleFeature(intImg, HaarlikeType(feature_description[0]),
                                                                                           feature_description[1],
                                                                                           feature_description[2],
                                                                                           feature_description[3],
                                                                                           feature_description[4], )
            X[i] = list(
                map(lambda data: feature_extractor(data[0]), training_data, ))
            i += 1
        return X, y
...
class WeakClassifier:
    ...
    def classify(self, intImg):
        def feature_extractor(intImg): return HaarlikeFeatureDetector.getSingleFeature(intImg, HaarlikeType(
...
# replace [(integeral_img(origMat), 1)] with training data
haar.apply_features(features, [(integeral_img(origMat), 1)])
